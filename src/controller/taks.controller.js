...
const addTask = asyncHandler(async (req, res) => {
   ...
   if (matchingDailyPointIndex !== -1) {
      const matchingTaskIndex = streak.dailyPoint[matchingDailyPointIndex].task.findIndex((item) => {
         return item.taskName === task;
      });

      if (matchingTaskIndex !== -1) {
         streak.dailyPoint[matchingDailyPointIndex].task[matchingTaskIndex].point = point;
      } else {
         streak.dailyPoint[matchingDailyPointIndex].task.push({ taskName: task, point: point });
      }
   } else {
      streak.dailyPoint.push({
         date: currentDate,
